...
structure and computes some aggregate value\hl{s?}. The inspiration for
...
The above code shows a simple type of expressions, represented
...
For instance, accumulate results in a set, we could use the following \lstinline{SetMonoid}:

\lstinputlisting[linerange=27-36]{../ObjectAlgebras/src/monoid/SetMonoid.java} % APPLY:linerange=SET_MONOID


Using the monoid operations \hl{alone}, it is possible to write a ``generic
