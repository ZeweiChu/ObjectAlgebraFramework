...
The above code shows a variant of the type of expressions shown in 
Section~\ref{subsec:ObjectAlgebras}, 
represented as the object algebra interface \lstinline{ExpAlg}. We will use this
...
implementation of the \lstinline{Monoid} interface:
...
Fig.~\ref{freevars_without_monoid}. The first difference is that the monoid to be used
...
it is highly reusable! For example, \lstinline{SetMonoid} could be reused 
in a query that collects all literals, or other information about the AST. 
In fact, because monoid instances are so general purpose, the \name library 
already contains many common monoid implementations. So users do not have to 
define these instances themselfes.
The second difference is that only the case for variables needs to
