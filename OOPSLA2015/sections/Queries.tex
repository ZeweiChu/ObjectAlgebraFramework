...
define the free variables operation. Instead of directly defining the
free variables operation, \lstinline{ExpAlgQuery} can be inherited, 
provided that the method \lstinline{m()} is implemented.
In the case of free variables, the monoid returned by \lstinline{m()} is an
implementation of the \lstinline{Monoid} interface for sets:
...
The general template in \name for an
algebra \lstinline[mathescape=true]{Alg<X$_1$, $...$, X$_n$>}, with
constructors \lstinline[mathescape=true]{f$_1$, $...$, f$_m$} is shown next:
...
\end{lstlisting}
\bruno{Should we have something that shows when empty
  is generated?}
