...
\label{deps2}
\end{figure}\bruno{This code {\bf must} instantiate the monoids.}
...
\lstinputlisting[linerange=14-21]{../ObjectAlgebras/src/deps2/TestDepGraph.java} % APPLY:linerange=GEN_DEPGRAPH

\noindent Note that here the argument of
\lstinline{makeExp}\bruno{{\bf makeExp} is the wrong name! It creates
  a statement, not an expression!} must
implement both \lstinline{ExpAlg} and \lstinline{StatAlg}. To achieve
this in Java, \lstinline{makeExp} has a type parameter \lstinline{A}
which is required to implement both interfaces. Using
\lstinline{makeExp}\bruno{wrong name} we can pass an instance of 
\lstinline{DepGraph} to compute the dependencies of the statement.

\lstinputlisting[linerange=32-32]{../ObjectAlgebras/src/deps2/TestDepGraph.java} % APPLY:linerange=CLIENTCODE_DEPGRAPH
\bruno{Show how to create depGraph as well.}
