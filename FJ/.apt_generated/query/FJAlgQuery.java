...
public interface FJAlgQuery<R> extends FJAlg<R, R, R, R, R, R, R, R, R, R> {
	...
	default R ClassDec(java.lang.String p0, java.util.List<R> p1, R p2, java.util.List<R> p3, R p4, java.util.List<R> p5) {
		R res = m().empty();
	...
	default R ConstrDec(java.lang.String p0, java.util.List<R> p1) {
		R res = m().empty();
		res = m().join(res, m().fold(p1));
		return res;
	}
	...
	default R MethodDec(java.util.List<R> p0, R p1, java.lang.String p2, java.util.List<R> p3, R p4) {
		R res = m().empty();
		res = m().join(res, m().fold(p0));
		res = m().join(res, p1);
		res = m().join(res, m().fold(p3));
