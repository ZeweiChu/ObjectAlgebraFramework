...
\end{lstlisting}\bruno{SalaryBill is probably a better name.}
...
\end{figure}\bruno{Rename FloatQuery to SalaryBill}
...
\bruno{We need client code in the paper! Show how to create a company
  structure (at least part of the code), and show how to use the
  algebras.}

\bruno{Avoid using qualifiers, such as \lstinline{private} and
  \lstinline{public}: they just make the code longer and harder to read.}
  
\haoyuan{Begin: client code.}

Some client code is shown as follows, using those two algebras based on our framework.

\lstinputlisting[linerange=10-21]{../ObjectAlgebras/src/example_SybAlg/SybTest.java} % APPLY:linerange=GEN_COM
