...
\begin{figure}[!htbp]
\lstinputlisting[linerange=3-7]{../ObjectAlgebras/src/expDemo2/ExpAlg.java} % APPLY:linerange=EXP_ALG
\vspace{-.1in}
...
those variables. In that case, a \lstinline{Var(s)} would simply return a singleton of \lstinline{s}, and a \lstinline{Lit(i)} corresponds to an empty set,
...
\begin{figure}[tb]
\lstinputlisting[linerange=3-18]{../ObjectAlgebras/src/expDemo2/FreeVarsExpAlg.java} % APPLY:linerange=FREEVARS_WITHOUT_MONOID
\vspace{-.1in}
...
\begin{figure}[!htbp]
\lstinputlisting[linerange=5-15]{../ObjectAlgebras/src/expDemo3/Monoid.java} % APPLY:linerange=MONOID
\vspace{-.1in}
...
\begin{figure}[tb]
\lstinputlisting[linerange=3-19]{../ObjectAlgebras/src/expDemo3/ExpAlgQuery.java} % APPLY:linerange=GENERIC_QUERY
\vspace{-.1in}
...
As an alternative way to handle the freeVars query, the query algebra is going to be a sub-interface of \lstinline{ExpAlgQuery}, the generic algebra, with generic type to be \lstinline{String[]}. To use the generic traversal code, a monoid is defined in Fig.~\ref{freevars_monoid}.

\begin{figure}[tb]
\lstinputlisting[linerange=3-15]{../ObjectAlgebras/src/expDemo3/FreeVarsMonoid.java} % APPLY:linerange=FREEVARS_MONOID
\vspace{-.1in}
...
But the result for an expression can only be a null array based on the monoid. Thus in the freeVars query, furthermore, we expect the variables to store their names into an array, and by using the monoid, freeVars can be implemented by:

\begin{figure}[!htbp]
\lstinputlisting[linerange=3-6]{../ObjectAlgebras/src/expDemo3/FreeVarsExpAlg.java} % APPLY:linerange=FREEVARS_WITH_MONOID
\vspace{-.1in}
